...
/***************************************************************************/
{
  ...
  tft.setTextDatum(TC_DATUM);  // XRate History legend
}
/***************************************************************************/
void loop()
/***************************************************************************/
{
  ...
  drawMultiBlendLine(&tft, 10, 10, 210, 110, blendSequence,
                     blendCt);
  doDebugPrints = false;
  drawMultiBlendLine2(&tft, 10, 30, 300, 210, blendSequence,
                      blendCt, true);

  Serial.println("<-Slope down-right longer ------------------->\r\n");
  doDebugPrints = false;
  drawMultiBlendLine2(&tft, 10, 50, 210, 210, blendSequence,
                      blendCt, false);

  Serial.println("<-Slope down-right v2 ----------------------->\r\n");
  doDebugPrints = false;
  drawMultiBlendLine2(&tft, 0, tft.height() / 4,
                      tft.width(), tft.height() / 4,
                      blendSequence, blendCt, true);

  Serial.println("<-Vertical down at 1/4 ---------------------->\r\n");
  doDebugPrints = false;
  drawMultiBlendLine2(&tft, tft.width() / 4, 0,
                      tft.width() / 4, tft.height(),
                      blendSequence, blendCt, true);

  Serial.println("<-Horizontal to left at .75 ----------------->\r\n");
  doDebugPrints = false;
  drawMultiBlendLine2(&tft, tft.width(), tft.height() * 0.75,
                      0, tft.height() * 0.75,
                      blendSequence, blendCt, true);

  Serial.println("<-Vertical upwards at .75 ------------------->\r\n");
  doDebugPrints = false;
  drawMultiBlendLine2(&tft, tft.width() * 0.75, tft.height(),
                      tft.width() * 0.75, 0,
                      blendSequence, blendCt, true);
  ...
  doDebugPrints = false;
  drawMultiBlendLine2(&tft, tft.width(), tft.height() * 0.75, 0, 0,
                      blendSequence, blendCt, true);

  Serial.println("<-Random ------------------------------------>\r\n");
  doDebugPrints = false;
  for (i = 0; i < 5; i++) {
    drawMultiBlendLine2(&tft, random(tft.width()), random(tft.height()),
                        random(tft.width()), random(tft.height()),
                        blendSequence, blendCt, true);
  }
 
  tft.drawString("Done.", tft.width() / 2, 0, 4);
  delay(5000);
  tft.fillScreen(TFT_BLACK);
}
/***************************************************************************/
